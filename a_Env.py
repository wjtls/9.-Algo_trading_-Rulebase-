...
class Env:
    ...
    def input_create(self, minute,  # minute:분봉
        ...
        for ind_name in long_ind_name + short_ind_name:
            ...
            if ind_name=='CCI_trend':
                CCI_t,ori_CCI_t = ind.CCI_trend(open_,2,100) #CCI_period, log_period
                ind_list.append(CCI_t)
                ori_ind_list.append(ori_CCI_t)

            if ind_name == 'NNCO_up_S':
                NCO_up, ori_up = ind.NNCO_up(open_, 40, 10)  # 지지 저항 Long
                ind_list.append(NCO_up)
                ori_ind_list.append(ori_up)

            if ind_name == 'NNCO_down_S':
                NCO_down, ori_down = ind.NNCO_down(open_, 40, 10)
    ...
    def long_discrete_step(self, action, unit, step,model):  # 에이전트의 액션이 discrete할때 보상을 결정하는 함수, 롱온리
        ...
        if action == 0:  # 롱 청산
            if model.long_aver_price == 0 and model.long_unit == 0:  # 숏포지션에서 산게 없는경우
                model.cash += (model.long_unit * model.deposit)
                model.long_unit = 0
                unit[0] = 0
                model.long_price = []
                model.long_aver_price = 0
                ...
                if model.long_unit < unit[0]:
                    ...
                    model.cash += (model.deposit * unit[0]) + (model.price - model.long_aver_price) * unit[0]  # 전량매도
            ...
            if params.train_stock_or_future =='future':
                if model.cash >= unit[2] * model.deposit:  # 가진현금이 사려고하는 가격보다 많을때
                    model.cash -= model.deposit * unit[2]
                    model.long_price.append((model.price + model.slip) * unit[2])
                    model.long_unit += unit[2]
        ...
        # 예외상황 전부 롱인데 맨처음이 상승이었고 계속 하락인경우 리워드는? PV는 감소하는데 리워드는 관망
        # 맨처음 롱 풀매수하면 리워드 상승, 더이상 살돈이 없는경우 그냥 리워드는 높은채, PV는 감소
        if params.traj_print == True:
            print(action, '액션', reward, '리워드', model.PV, 'PV', model.cash, 'cash', model.long_unit, 'long_stock',
                  model.short_unit, '숏유닛',
                  unit, '유닛', unit, '리워드계산시유닛', model.Agent_num, '에이전트넘버', model.price, '가격',
    ...
    def short_discrete_step(self, action, unit, step,model):
        ...
        if action == 0:  # 숏 청산
            ...
            elif model.short_unit != 0:  # 숏 포지션 보유중인경우
                if model.short_unit < unit[0]:
                    # 가진계약 부족한데 팔계약은 있는경우 전량 청산

                    unit[0] = model.short_unit
                    model.cash += (model.deposit * unit[0]) + (model.short_aver_price - model.price) * unit[
                        0]  # 전량매도
                    ...
                    model.short_unit -= unit[0]
                    model.cash += (unit[0] * model.deposit) + (model.short_aver_price - model.price) * unit[0]
