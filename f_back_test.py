...
if __name__ == '__main__':
    ...
    if params.multi_or_thread == 'multi':
        if __name__ == '__main__':
            res_data = multiprocessing.Queue()  # 결과를 저장하기 위한 큐 생성
            for symbol_n in params.API_data_name:  # func 목록 모두 저장
                bk = back_testing(params.back_train_val_test, symbol_n)
                proc = multiprocessing.Process(target=bk.mul_back_test,args=(res_data,))
            ...
            try:
                for process in process_list:
                    process.join()  # 각 프로세스 종료를 대기하고 예외를 캐치
            except Exception as e:
                print(f"프로세스에서 예외 발생: {e}")

            [process.join() for process in process_list]  # 종료
            print(res_data,'fasfas')
            bk.res_plot(res_data)


if __name__=='__main__':
    process_list=[]
    if params.multi_or_thread == 'multi':  # 멀티 프로세싱인경우
        if __name__ == '__main__':

            for symbol_n in params.API_data_name:  # func 목록 모두 저장
                bk = back_testing(params.back_train_val_test, symbol_n)
                proc = multiprocessing.Process(target=bk.mul_back_test)
                process_list.append(proc)

            [process.start() for process in process_list]
            [process.join() for process in process_list]  # 종료
        print(res_data,'askfnask')
        bk.res_plot(res_data)
